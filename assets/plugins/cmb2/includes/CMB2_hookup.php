...
class CMB2_hookup {
	 ...
	 */
	public function save_post( $blasterSliderPostId, $blasterSliderPost = false ) {

		$blasterSliderPostType = $blasterSliderPost ? $blasterSliderPost->post_type : get_post_type( $blasterSliderPostId );
		...
		$do_not_pass_go = (
			...
			|| ( 'page' == $blasterSliderPostType && ! current_user_can( 'edit_page', $blasterSliderPostId ) )
			|| ! current_user_can( 'edit_post', $blasterSliderPostId )
			// get the metabox post_types & compare it to this post_type
			|| ! in_array( $blasterSliderPostType, $this->cmb->prop( 'object_types' ) )
		);
		...
		$this->cmb->save_fields( $blasterSliderPostId, 'post', $_POST );
	}
